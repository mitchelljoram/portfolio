...
const LanguageTag = ({name, hex, dimension}: LanguageTagProps) => {
  ...
  const handleMouseOver = () => {
    hovered === false && setHovered(true)
  }
  const handleMouseLeave = () => {
    hovered === true && setHovered(false)
  }

  return (
    <div>
      <div width="175px" height="175px" background={`#${hex}40`} borderColor={`#${hex}`} borderWidth="2" cursor="default" onMouseOver={() => handleMouseOver()} onMouseLeave={() => handleMouseLeave()}>
      ...
      <div display={hovered ? "flex" : "none"} position="absolute" zIndex={10} width="100px" height="100px" background={`#${hex}bf`} backdropFilter="saturate(100%) blur(10px)" borderRadius="50%" justifyContent="center" left={hovered ? xy.x : null} top={hovered ? xy.y+scrollPosition : null} cursor="default">
        <div my="auto">
